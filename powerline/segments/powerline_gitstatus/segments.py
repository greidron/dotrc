...
class GitStatusSegment(Segment):
	...
	def has_error(err):
            return err and ('error' in err[0] or 'fatal' in err[0])

        status, err = self.execute(pl, base + ['status', '--branch', '--porcelain'])

        branch, detached, behind, ahead = (None, False, 0, 0)
        if has_error(err):
            status, err = self.execute(pl, base + ['status', '--porcelain'])
            if has_error(err):
		return
            branch_list, err = self.execute(pl, base + ['rev-parse', '--abbrev-ref', 'HEAD'])
            if has_error(err):
                return
            branch = branch_list[0]
            rev_list, err = self.execute(pl, base + ['rev-list', '--left-right', '@{{upstream}}...{0}'.format(branch)])
            if has_error(err):
		detached = True
            else:
		detached = False
